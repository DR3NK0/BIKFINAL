...
public class dialogueSystem : MonoBehaviour
{
    ...
    [SerializeField] AudioSource talkingSFXKids;
    ...
    [SerializeField] string targetHexColor;
    ...
    {
        return chatIndex == levels[currentLevel].Count - 1;
    }
            ...
            if (chars[i] == '<')
            {
                ...
                if (endIndex != -1)
                {
                    ...
                    if (tag.StartsWith("<color="))
                    {
                        ...
                        int closeIndex = tag.IndexOf('>', hashIndex);

                        if (hashIndex != -1)
                        {
                            string hex = tag.Substring(hashIndex, 7);

                            if (hex.Equals(targetHexColor, StringComparison.OrdinalIgnoreCase))
                            {
